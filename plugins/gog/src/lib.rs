// GOG Plugin for HappyView
// Uses GOG OAuth2 for authentication and GOG Galaxy API for data
...
struct GogUserData {
    ...
    galaxy_user_id: Option<String>,
    avatar: Option<String>,
}

#[derive(Deserialize)]
struct GogOwnedGamesResponse {
    owned: Vec<i64>,
}
...
const GOG_AUTHORIZE_URL: &str = "https://auth.gog.com/auth";
const GOG_TOKEN_URL: &str = "https://auth.gog.com/token";
const GOG_API_BASE: &str = "https://embed.gog.com";
...
pub extern "C" fn plugin_info() -> i64 {
    let info = PluginInfo {
        ...
        version: "0.1.0".into(),
        api_version: "1".into(),
        icon_url: Some("https://www.gog.com/favicon.ico".into()),
...
pub extern "C" fn handle_callback(input_ptr: u32, input_len: u32) -> i64 {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        let tokens: GogTokenResponse = match serde_json::from_str(&token_response) {
            Ok(t) => t,
...
pub extern "C" fn get_profile(input_ptr: u32, input_len: u32) -> i64 {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        let profile = ExternalProfile {
            ...
            profile_url: Some(format!("https://www.gog.com/u/{}", user.user_id)),
...
pub extern "C" fn sync_account(input_ptr: u32, input_len: u32) -> i64 {
    #[cfg(target_arch = "wasm32")]
    {
        ...
        // Fetch owned games
        let owned_url = format!("{}/user/data/games", GOG_API_BASE);
        let owned_body = match http_get_with_auth(&owned_url, &input.access_token) {
            Ok(b) => b,
            Err(e) => return return_error("HTTP_ERROR", &e, true),
        };

        let owned: GogOwnedGamesResponse = match serde_json::from_str(&owned_body) {
            Ok(o) => o,
            Err(e) => return return_error("INVALID_RESPONSE", &format!("Parse error: {}", e), false),
        };
        ...
        for game_id in owned.owned {
            ...
            records.push(SyncRecord {
                ...
                record: game_record,
                dedup_key: Some(format!("gog:game:{}", game_id)),
                sign: true,
            });
        }
